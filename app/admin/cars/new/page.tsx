...
export default function AddCarPage() {
  ...
  for (const file of files) {
    ...
    console.log('Uploading:', fileName) // 👈 check terminal
  ...
  if (!res.ok) { toast.error(data.error || 'Failed to add car') }
  else { toast.success('Car added!'); router.push('/admin/cars') }
}
    ...
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Fill in the details and upload photos</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-sm p-6 shadow-sm">
          ...
          <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-200 rounded-sm p-8 text-center cursor-pointer hover:border-brand-gold transition-colors">
            ...
            <p className="text-gray-400 text-xs mt-1">JPG, PNG, WEBP. First photo = main photo.</p>
        ...
        <div className="bg-white rounded-sm p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input name="title" value={form.title} onChange={handleChange} className="input-field" placeholder="e.g. Honda City ZX 2019 - Well Maintained" required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Brand *</label>
              <input name="brand" value={form.brand} onChange={handleChange} className="input-field" placeholder="e.g. Honda" required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Model *</label>
              <input name="model" value={form.model} onChange={handleChange} className="input-field" placeholder="e.g. City" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" name="price" value={form.price} onChange={handleChange} className="input-field" placeholder="e.g. 650000" required />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">KM Driven *</label>
              <input type="number" name="km_driven" value={form.km_driven} onChange={handleChange} className="input-field" placeholder="e.g. 45000" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input name="color" value={form.color} onChange={handleChange} className="input-field" placeholder="e.g. Pearl White" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Location</label>
              <input name="location" value={form.location} onChange={handleChange} className="input-field" placeholder="e.g. Delhi" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Description</label>
              <textarea name="description" value={form.description} onChange={handleChange} className="input-field resize-none" rows={4} placeholder="Describe the car condition, service history..." />
            </div>
            <div className="flex items-center gap-3">
              ...
              <label htmlFor="is_featured" className="text-sm text-gray-600">Mark as Featured (shows on homepage)</label>
            </div>
          </div>
        </div>
