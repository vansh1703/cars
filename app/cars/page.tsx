...
async function getCars(sp: any): Promise<Car[]> {
  let query = supabase.from('cars').select('*').eq('is_archived', false).eq('is_sold', false)
...
async function getBrands(): Promise<string[]> {
  const { data } = await supabase.from("cars").select("brand").eq('is_archived', false).eq('is_sold', false)
  if (!data) return []
