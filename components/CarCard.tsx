...
export default function CarCard({ car }: { car: Car }) {
  ...
  return (
    <Link href={`/cars/${car.id}`} className="card-car group block">
      {/* Image */}
      <div className="relative h-52 overflow-hidden bg-gray-100">
        ...
        {car.is_featured && !car.is_sold && (
          <div className="absolute top-3 left-3 bg-brand-gold text-brand-navy text-xs font-bold px-2 py-1 rounded-sm">
            FEATURED
          </div>
        )}
        {car.images?.length > 1 && (
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="text-xs bg-brand-cream text-brand-steel px-2 py-1 rounded-sm font-medium">
            {car.transmission}
          </span>
